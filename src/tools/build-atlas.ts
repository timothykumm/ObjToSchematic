...
program
  .argument(
    "<textures_directory>",
    "The directory to load the blocks texture files from (assets/minecraft/textures/block)"
  )
  .argument(
    "<models_directory>",
    "The directory to load the blocks model files from (assets/minecraft/models/block)"
  )
  .argument(
    "<output_directory>",
    "The directory to write the texture atlas files to"
  )
...
const ignoreList = new Set(
  fs.readFileSync(paths.ignore, "utf8").split(/\r?\n/)
);
...
const textureData = Array.from(allTextures).map(async (texture) => {
  ...
  const imageData = Uint8ClampedArray.from(
    await image.raw().ensureAlpha(1.0).toBuffer()
  ); // 16 x 16 x 4
...
Promise.all(textureData).then(async (res) => {
  const tmp = res
    .sort((a, b) => {
  ...
  Promise.all(tmp).then(async (data) => {
    const textureMap = new Map<
      string,
      Omit<ArrayElement<typeof data>, "textureName" | "texturePath">
    >();
    ...
    fs.writeFileSync(
      path.join(paths.output, "atlas.atlas"),
      JSON.stringify(atlasFile, null, 4)
    );
  });
});
