...
export class AppAnalytics {
    ...
    public static Init() {
        install('G-W0SCWQ7HGJ', { 'send_page_view': true });
        gtag('js', new Date());
        gtag('config', 'G-W0SCWQ7HGJ', AppConfig.Get.VERSION_TYPE === 'd' ? { 'debug_mode': true } : {});
    ...
    public static Event(id: string, attributes?: any) {
        if (this.Get._ready) {
            ...
            gtag('event', id, Object.assign(attributes ?? {}, AppConfig.Get.VERSION_TYPE === 'd' ? { 'debug_mode': true } : {}));
        }
    }
}