...
export class UI {
  ...
  public _ui = {
    ...
    import: {
      ...
      components: {
        input: new FileComponent().setLabel("import.components.input"),
    ...
    materials: {
      ...
      components: {
        input: new TexturedMaterialComponent("Material.001", SolidMaterial),
      },
      componentOrder: [],
      execButton: new ButtonComponent()
        .setOnClick(() => {
    ...
    voxelise: {
      ...
      components: {
        constraintAxis: new ComboboxComponent<TAxis>()
          .addItem({
            payload: "y",
            displayLocKey: "voxelise.components.y_axis",
          })
          .addItem({
            payload: "x",
            displayLocKey: "voxelise.components.x_axis",
          })
          .addItem({
            payload: "z",
            displayLocKey: "voxelise.components.z_axis",
          })
          ...
          .addValueChangedListener((value: TAxis) => {
            switch (value) {
              case "x": {
                ASSERT(
                  this._appContext !== undefined &&
                    this._appContext.minConstraint !== undefined &&
                    this._appContext.maxConstraint !== undefined
                );
                console.log(
                  "min",
                  this._appContext.minConstraint,
                  "max",
                  this._appContext.maxConstraint
                );
                this._ui.voxelise.components.size.setMin(
                  this._appContext.minConstraint.x
                );
                this._ui.voxelise.components.size.setMax(
                  this._appContext.maxConstraint.x
                );
                break;
              }
              case "y": {
                this._ui.voxelise.components.size.setMin(
                  AppConfig.Get.CONSTRAINT_MINIMUM_HEIGHT
                );
                this._ui.voxelise.components.size.setMax(
              ...
              case "z": {
                ...
                this._ui.voxelise.components.size.setMin(
                  this._appContext.minConstraint.z
                );
                this._ui.voxelise.components.size.setMax(
                  this._appContext.maxConstraint.z
                );
                break;
              }
            }
        ...
        voxeliser: new ComboboxComponent<TVoxelisers>()
          .addItem({
            payload: "ray-based",
            displayLocKey: "voxelise.components.ray_based",
          })
          .addItem({
            payload: "bvh-ray",
            displayLocKey: "voxelise.components.bvh_ray",
          })
          .addItem({
            payload: "ncrb",
            displayLocKey: "voxelise.components.ncrb",
          })
        ...
        placeholder: new PlaceholderComponent().setPlaceholderText(
          "misc.advanced_settings"
        ),
      },
      componentOrder: [
        "constraintAxis",
    ...
    assign: {
      ...
      components: {
        textureAtlas: new ComboboxComponent<string>()
          .addItem({
            displayLocKey: "assign.components.vanilla",
            payload: "vanilla",
          })
          .setLabel("assign.components.texture_atlas")
          .setShouldObeyGroupEnables(false),
        blockPalette: new PaletteComponent().setLabel(
          "assign.components.block_palette"
        ),
        dithering: new ComboboxComponent<TDithering>()
          ...
          .addEnabledChangedListener((isEnabled) => {
            this._ui.assign.components.ditheringMagnitude.setEnabled(
              isEnabled &&
                this._ui.assign.components.dithering.getValue() !== "off",
              false
            );
          })
        ...
        calculateLighting: new CheckboxComponent()
          ...
          .addValueChangedListener((newValue: boolean) => {
            const isEnabled =
              this._ui.assign.components.calculateLighting.getEnabled();
            this._ui.assign.components.lightThreshold.setEnabled(
              newValue && isEnabled,
              false
            );
          })
          .addEnabledChangedListener((isEnabled: boolean) => {
            const value =
              this._ui.assign.components.calculateLighting.getValue();
            this._ui.assign.components.lightThreshold.setEnabled(
              value && isEnabled,
              false
            );
    ...
    groups: {
      viewmode: {
        components: {
          ...
          voxelMesh: new ToolbarItemComponent({
            id: "voxelMesh",
            iconSVG: AppIcons.VOXEL,
          })
            .onClick(() => {
          ...
          blockMesh: new ToolbarItemComponent({
            id: "blockMesh",
            iconSVG: AppIcons.BLOCK,
          })
            .onClick(() => {
      ...
      debug: {
        components: {
          ...
          "night-vision": new ToolbarItemComponent({
            id: "night",
            iconSVG: AppIcons.BULB,
          })
            .onClick(() => {
      ...
      sliceHeight: {
        components: {
          slice: new ToolbarItemComponent({
            id: "slice",
            iconSVG: AppIcons.SLICE,
          })
            .onClick(() => {
          ...
          plus: new ToolbarItemComponent({ id: "plus", iconSVG: AppIcons.PLUS })
            ...
            .isEnabled(() => {
              return (
                Renderer.Get.isSliceViewerEnabled() &&
                Renderer.Get.canIncrementSliceHeight()
              );
            })
            .setTooltip("toolbar.decrement_slice"),
          minus: new ToolbarItemComponent({
            id: "minus",
            iconSVG: AppIcons.MINUS,
          })
            .onClick(() => {
  ...
  private _toolbarRight = {
    groups: {
      zoom: {
        components: {
          zoomOut: new ToolbarItemComponent({
            id: "zout",
            iconSVG: AppIcons.MINUS,
          })
            .onClick(() => {
          ...
          zoomIn: new ToolbarItemComponent({
            id: "zin",
            iconSVG: AppIcons.PLUS,
          })
            .onClick(() => {
              ArcballCamera.Get.onZoomIn();
            })
            .setTooltip("toolbar.zoom_in"),
          reset: new ToolbarItemComponent({
            id: "reset",
            iconSVG: AppIcons.CENTRE,
          })
            .onClick(() => {
      ...
      camera: {
        components: {
          perspective: new ToolbarItemComponent({
            id: "pers",
            iconSVG: AppIcons.PERSPECTIVE,
          })
            .onClick(() => {
          ...
          orthographic: new ToolbarItemComponent({
            id: "orth",
            iconSVG: AppIcons.ORTHOGRAPHIC,
          })
            .onClick(() => {
  ...
  private _toolbarLeftDull: { [key: string]: ToolbarGroup } =
    this._toolbarLeft.groups;
  private _toolbarRightDull: { [key: string]: ToolbarGroup } =
    this._toolbarRight.groups;
  ...
  public tick(isBusy: boolean) {
    ...
    if (
      ArcballCamera.Get.isUserRotating ||
      ArcballCamera.Get.isUserTranslating
    ) {
  ...
  private _forEachComponent(
    action: EAction,
    functor: (component: ConfigComponent<unknown, unknown>) => void
  ) {
  ...
  public updateMaterialsAction(materialManager: MaterialMapManager) {
    ...
    if (materialManager.materials.size == 0) {
      this.layoutDull["materials"].components[`placeholder_element`] =
        new PlaceholderComponent().setPlaceholderText(
          "materials.components.no_materials_loaded"
        );
      ...
      materialManager.materials.forEach((material, materialName) => {
        if (material.type === MaterialType.solid) {
          this.layoutDull["materials"].components[`mat_${materialName}`] =
            new SolidMaterialComponent(materialName, material)
              .setUnlocalisedLabel(materialName)
              .onChangeTypeDelegate(() => {
                materialManager.changeMaterialType(
                  materialName,
          ...
          this.layoutDull["materials"].components[`mat_${materialName}`] =
            new TexturedMaterialComponent(materialName, material)
              .setUnlocalisedLabel(materialName)
              .onChangeTypeDelegate(() => {
                materialManager.changeMaterialType(
                  materialName,
              ...
              .onChangeTransparencyTypeDelegate((newTransparency) => {
                materialManager.changeTransparencyType(
                  materialName,
                  newTransparency
                );
                this.updateMaterialsAction(materialManager);
