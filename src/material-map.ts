...
import { EImageChannel, TTransparencyTypes, TImageRawWrap, TTransparencyOptions } from './texture'; // Added TImageRawWrap, TTransparencyOptions
...
export class MaterialMapManager {
    ...
    public updateMaterialProperty(materialName: string, propertyName: string, value: any) {
        ...
        if (!material) {
            console.error(`[MaterialMapManager] Material '${materialName}' not found for update.`);
            return;
        }

        console.log(`[MaterialMapManager] Updating '${propertyName}' for material '${materialName}' with value:`, value);
        ...
        if (material.type === MaterialType.textured) {
            ...
            switch (propertyName) {
                ...
                case 'transparency':
                    ...
                    const uiTransparencySettings = value as { type: TTransparencyTypes; alphaValue?: number; alphaMap?: TImageRawWrap | null; alphaChannel?: EImageChannel; };
                    let coreTransparencyOption: TTransparencyOptions;

                    if (uiTransparencySettings.type === 'UseAlphaValue') {
                        coreTransparencyOption = {
                            type: 'UseAlphaValue',
                            alpha: uiTransparencySettings.alphaValue !== undefined ? uiTransparencySettings.alphaValue : 1.0
                        };
                    ...
                    } else { // Handles 'None' and 'UseDiffuseMapAlphaChannel'
                        coreTransparencyOption = { type: uiTransparencySettings.type };
                    }
                    texturedMaterial.transparency = coreTransparencyOption;
                    break;
                default:
                    console.warn(`[MaterialMapManager] Unhandled property '${propertyName}' for textured material '${materialName}'.`);
            ...
            switch (propertyName) {
                case 'colour':
                    ...
                    break;
                default:
                    console.warn(`[MaterialMapManager] Unhandled property '${propertyName}' for solid material '${materialName}'.`);
                    material.needsAttention = false; // Revert
                    return;
            }
        } else {
            console.error(`[MaterialMapManager] Unknown material type for '${materialName}'. Type: ${material.type}`);
            material.needsAttention = false; // Revert
            return;
        }
        ...
        console.log(`[MaterialMapManager] Material '${materialName}' updated:`, material);
    }
}
