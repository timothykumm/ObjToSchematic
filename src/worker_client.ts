...
import {
  ...
  InitParams,
  RenderMeshParams,
  RenderNextBlockMeshChunkParams,
  RenderNextVoxelMeshChunkParams,
  SetMaterialsParams,
  SettingsParams,
  TFromWorkerMessage,
  VoxeliseParams,
...
export class WorkerClient {
  ...
  ): Promise<ImportParams.Output> {
    const parsed = path.parse(params.file.name);

    const importer = ImporterFactor.GetImporter(
      parsed.ext === ".obj" ? "obj" : "gltf"
    );
  ...
  public voxelise(params: VoxeliseParams.Input): VoxeliseParams.Output {
    ...
    const voxeliser: IVoxeliser = VoxeliserFactory.GetVoxeliser(
      params.voxeliser
    );
  ...
  public assign(params: AssignParams.Input): AssignParams.Output {
    ...
    this._loadedBlockMesh = BlockMesh.createFromVoxelMesh(
      this._loadedVoxelMesh,
      params
    );
  ...
  public renderChunkedBlockMesh(
    params: RenderNextBlockMeshChunkParams.Input
  ): RenderNextBlockMeshChunkParams.Output {
    ...
    const buffer = this._loadedBlockMesh.getChunkedBuffer(
  ...
  public export(params: ExportParams.Input): ExportParams.Output {
    ...
    if (!exporter) {
      throw new Error(
        `WorkerClient: Exporter object is unexpectedly undefined for type '${params.exporter}' before calling export.`
      );
    }
    // ---------------------------------
